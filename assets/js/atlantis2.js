...
function legendClickCallback(event) {
  ...
  var chartId = parseInt(
    parent.classList[0].split("-")[0],
    10
  );
...
$(document).ready(function () {
  ...
  var messagesScrollbar = $(
    ".messages-scroll.scrollbar-outer"
  );
  ...
  if (!toggle_sidebar && $(".sidenav-toggler").length) {
    ...
    toggle.on("click", function () {
      if (nav_open == 1) {
        ...
        $('<div class="body-overlay"></div>').insertAfter(
          ".nav-bottom"
        );
  ...
  if (!quick_sidebar_open) {
    ...
    toggle.on("click", function () {
      if (nav_open == 1) {
        ...
        toggle.addClass("toggled");
        $(
          '<div class="quick-sidebar-overlay"></div>'
        ).insertAfter(".quick-sidebar");
...
$('.input-file-image input[type="file"').change(
  function () {
    readURL(this);
  }
);
